...
**Ricerca basata sugli indici**: poiché nella maggior parte dei documenti effettuare una ricerca direttamente all'interno dei documenti stessi risulterebbe troppo lento, DocFetcher richiede la creazione di *indici* delle cartelle all'interno delle quali si vogliono effettuare ricerche. Questi indici consentono a DocFetcher di recuperare facilmente i file e il loro contenuto attraverso delle parole-chiave, in modo del tutto simile a quanto si fa quando si consulta l'indice presente nelle ultime pagine di un libro. La creazione di un indice può richiedere un certo tempo che dipende dal numero e dalle dimensioni dei file che debbono essere indicizzati all'interno della cartella. In ogni caso questo processo va eseguito una sola volta per ciascuna cartella; poi si possono effettuare delle ricerche all'interno delle cartelle indicizzate quante volte che si vuole.

**Creazione di un indice**: per creare un indice bisogna fare clic col pulsante destro del mouse nell'area `Ambito di ricerca` posta in basso a sinistra e selezionare `Crea Indice da > Cartella`. A questo punto bisogna scegliere la cartella da indicizzare. All'inizio è opportuno selezionare una cartella contenente un numero non troppo elevato di file, per esempio una cinquantina circa. Dopo aver selezionato la cartella, compare una nuova finestra al cui interno sono presenti i parametri di configurazione dell'indicizzazione. I valori predefiniti dovrebbero andare bene, per cui è sufficiente fare clic sul pusante `Esegui` e aspettare che DocFetcher completi l'indicizzazione. Un metodo alternativo per creare un indice è quello di selezionare e copiare una cartella a partire da Esplora risorse. Successivamente bisogna fare clic con il pulsante destro del mouse nell'area `Ambito di ricerca` e selezionare `Crea Indice da > Appunti`.

**Ricerca**: per effettuare una ricerca bisogna inserire una o più parole nel campo di testo posizionato sopra il pannello dei risultati (cioè la tabella avente le intestazioni per le colonne) e premere il tasto `Invio` o fare clic col mouse sopra il pulsante `Cerca`. I risultati della ricerca compariranno nell'omonimo riquadro, ordinati per punteggio decrescente.

*Se si sta leggendo questa guida all'interno di DocFetcher e si seguono le istruzioni del paragrafo successivo la guida scomparirà. Per ripristinarla, bisogna fare clic sul pulsante `'?'` in alto a destra. È anche possibile aprire la guida nel proprio browser predefinito facendo clic sul pulsante `Apri nel browser esterno` posto sopra il presente pannello.*

**Pannello dei risultati e pannello di anteprima**: al di sotto del pannello dei risultati (o alla sua destra  &mdash; in funzione di come è organizzato il layout dell'interfaccia-utente), si trova il pannello di anteprima. Selezionando un file nel pannello dei risultati, in quello di anteprima verrà mostrata un'anteprima del contenuto del file in questione, in formato testo. Caratteristiche importanti sono:

* ***Evidenziazione***: in modalità predefinita, i termini di ricerca utilizzati vengono evidenziati e si può saltare da una occorrenza alla precedente o alla successiva usando semplicemente i pulsanti "Freccia Su" (occorrenza precedente) e "Freccia Giù" (occorrenza successiva).
...
**Filtri**:  nella parte sinistra dell'interfaccia-utente sono disponibili diversi sistemi per filtrare i risultati: (1) A livello di `Dimensione minima/massima del file` è possibile specificare una dimensione minima e/o massima dei file ai quali si è interessati. (2) La voce `Tipi di documento` consente di filtrare i risultati per tipo. (3) Deselezionando una o più caselle di voci presenti nell'area `Ambito di ricerca`, è possibile filtrare i risultati anche in base a una o più cartelle indicizzate (e alle relative sottocartelle).
...
1. ***DocFetcher di per sé***: se DocFetcher è in esecuzione e la *sorveglianza delle cartelle* per eventuali modifiche a tale livello è abilitata, DocFetcher rileva le variazioni e aggiorna i propri indici immediatamente.
...
2. ***Problemi di memoria***: DocFetcher deve mantenere in memoria una rappresentazione dei file che deve essere la più ridotta possibile. A causa di ciò e poiché le cartelle di sistema generalmente contengono un gran numero di file, se vengono indicizzate, DocFetcher può andare con relativa facilità *out of memory*.
...
**Supporto Unicode**: DocFetcher supporta pienamente Unicode in tutti i formati di documento eccetto il caso dei file CHM. Nel caso di file di testo semplice, DocFetcher deve usare un [approccio euristico](http://www-archive.mozilla.org/projects/intl/UniversalCharsetDetection.html) per tentare di indovinare la codifica corretta, in quanto i file di testo semplice non possiedono alcuna informazione esplicita riguardante la loro codifica.

**File CHM**: i file CHM non sono supportati da Java a 64-bit e dal sistema operativo Mac OS&nbsp;X. Ciò significa che per poter supportare i file CHM in Windows e in Linux potrebbe essere necessario rimpiazzare la versione runtime di Java a 64-bit con la sua controparte a 32-bit.
...
* [Espressioni regolari (RegEx)](DocFetcher_Manual_files/Regular_Expressions.html)
* [Notifica del rilascio di nuove versioni del programma](DocFetcher_Manual_files/Release_Notification.html)
* [Come aumentare la quantità di memoria](DocFetcher_Manual_files/Memory_Limit.html)
